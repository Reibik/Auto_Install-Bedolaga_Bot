...
Через меню **9 → 10**. Полезно для серверов с 1 ГБ RAM:
...
Используйте функцию **миграции** (меню **9 → 12**): экспорт конфигов → копирование на новый сервер → импорт.

### Серверу не хватает RAM

Создайте swap через меню **9 → 10**. Рекомендуется 2 ГБ для серверов с 1 ГБ RAM.
